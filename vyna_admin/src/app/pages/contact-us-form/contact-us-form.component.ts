...
searchText: string = '';
  ...
  getUser(pageIndex: number, pageSize: number) {
    ...
    this.userService.contactusfrom(pageIndex + 1, pageSize).subscribe(
      (response: any) => {
  ...
  DeleteAccount(id: any): void {
    if (confirm('Are you sure you want to delete form?')) {
      this.userService.DeleteForm(id).subscribe(
        (response: any) => {
...
truncateMessage(message: string, limit: number = 10): string {
