...
const cmsBannerModel = require("../../../models/cmsBannerModel");
...
exports.create = catchAsync(async (req, res, next) => {
  try {
    ...
    let check = await cmsBannerModel.findById(id);
    ...
    let data = await cmsBannerModel.create(body);
...
exports.getAllBanner = async (req, res, next) => {
  try {
    ...
    let user = await cmsBannerModel
      .find()
      .skip(skip)
      .limit(limit)
     // .sort({ _id: -1 });
    const totalCount = await cmsBannerModel.countDocuments();
...
exports.getByID = async (req, res, next) => {
  try {
    ...
    let user = await cmsBannerModel.findById(id);
...
exports.update = async (req, res, next) => {
  try {
    ...
    let user = await cmsBannerModel.findByIdAndUpdate(id, body, { new: true });
...
exports.deleteByID = async (req, res, next) => {
  try {
    const id = req.params.id;
    const deletedCategory = await cmsBannerModel.findByIdAndDelete(id);
